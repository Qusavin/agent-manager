...
func TestJSONIsPromisedOnlyWhereTheCommandTakesIt(t *testing.T) {
	...
	cases := []struct {
		...
		{usageIssue, func(out *bytes.Buffer, args []string) error {
			return runIssue(out, sampleReporter(), append([]string{"t", "--body", "b"}, args...), "cafe0001")
		}},
...
func TestCommandsAndHelpCoverEverySection(t *testing.T) {
	...
	registered := []string{
		...
		"groups", "create-group", "delete-group", "task", "note", "reserve", "release-files", "reservations", "terminal",
	...
	help := Help("dev")
	for _, line := range []string{usageSessions, usageReserve, usageTerminalSend, usageRename, usageReviewComment, usageIssue, usageFeature, usageUpdate, "task <list|create|claim|finish|release|delete>", "note <read|append>"} {
